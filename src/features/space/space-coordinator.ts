...
export default abstract class SpaceCoordinator {
  ...
  toSpaceReservation(action: NavigationAction = "navigate", params: TTO_SPACE_RESERVAION_PROPS) {
    if (action === "replace")
      this.navigation.dispatch(StackActions.replace(this.screenName, { screen: "space-reservation", params }));
    else this.navigation.navigate(this.screenName, { screen: "space-reservation" });
  }

  toSpaceReserveConfirm(action: NavigationAction = "navigate", params: TTOCONFIRM__SPACE_RESERVAION_PROPS) {
    if (action === "replace")
      this.navigation.dispatch(StackActions.replace(this.screenName, { screen: "confirm-space-reservation", params }));
    else this.navigation.navigate(this.screenName, { screen: "confirm-space-reservation" });
  }
}
